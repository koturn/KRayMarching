Shader "koturn/KRayMarching/Primitives/VerticalCappedCone"
{
    Properties
    {
        // ---------------------------------------------------------------------
        ...
        [Space(8)]
        _Height ("Height of Cone", Float) = 0.3

        _Radius1 ("Bottom radius", Float) = 0.1

        _Radius2 ("Top radius", Float) = 0.2
    ...
    SubShader
    {
        ...
        float map(float3 p)
        {
            return sdCappedCone(p, _Height, _Radius1, _Radius2);
        }
        ENDCG
    ...
    CustomEditor "Koturn.KRayMarching.Inspectors.Primitives.VerticalCappedConeGUI"
}
