Shader "koturn/KRayMarching/Primitives/Ellipsoid"
{
    Properties
    {
        // ---------------------------------------------------------------------
        ...
        _AccelarationFactor ("Coefficient of Accelarating Sphere Tracing", Range(0.0, 1.0)) = 0.4

        _AutoRelaxFactor ("Coefficient of Automatic Step Size Relaxation", Range(0.0, 1.0)) = 0.4
        ...
        [Space(8)]
        [Vector3]
        _Radiuses ("Radiuses of the Ellipsoid", Vector) = (0.5, 0.5, 0.1, 0.0)
    ...
    SubShader
    {
        ...
        UNITY_DEFINE_INSTANCED_PROP(float3, _Radiuses)
        ...
        float map(float3 p)
        {
            return sdEllipsoid(p, UNITY_ACCESS_INSTANCED_PROP(Props, _Radiuses));
        }
        ENDCG
    ...
    CustomEditor "Koturn.KRayMarching.Inspectors.Primitives.EllipsoidGUI"
}
